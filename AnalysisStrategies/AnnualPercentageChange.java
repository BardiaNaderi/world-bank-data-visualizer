...
public class AnnualPercentageChange implements Analysis {
	...
	public void processJSON(JsonArray[] jsonArray, String[] analysisType, String country) {
		...
		if (jsonArray[0].get(1).getAsJsonArray().get(sizeOfResults - 1).getAsJsonObject().get("value").isJsonNull())
			pastYear = 0;
		else
			pastYear = jsonArray[0].get(1).getAsJsonArray().get(sizeOfResults - 1).getAsJsonObject().get("value").getAsFloat();
		...
		for (int i = sizeOfResults - 2; i >= 0; i--) {
			...
			System.out.println("The annual percent change of " + analysisType[0] + " for " + country + " from " + (year - 1) + " to " + year + " is " + percentChange);
		}
	}
